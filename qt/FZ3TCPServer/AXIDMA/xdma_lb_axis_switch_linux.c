...
typedef struct {
    ...
    XDma_lb_axis_switch_uio_map maps[ MAX_UIO_MAPS ];
} XDma_lb_axis_switch_uio_info;

/***************** Variable Definitions **************************************/
static XDma_lb_axis_switch_uio_info uio_info;
...
static int uio_info_read_name(XDma_lb_axis_switch_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDma_lb_axis_switch_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDma_lb_axis_switch_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDma_lb_axis_switch_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDma_lb_axis_switch_Initialize(XDma_lb_axis_switch *InstancePtr, const char* InstanceName) {
	XDma_lb_axis_switch_uio_info *InfoPtr = &uio_info;
